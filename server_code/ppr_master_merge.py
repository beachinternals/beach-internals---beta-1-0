...
@anvil.server.callable
def backfill_weather_for_old_files_background(league, gender, year):
  return backfill_weather_for_old_files(league, gender, year)

  
@anvil.server.background_task
def backfill_weather_for_old_files(league, gender, year, team=None, dry_run=False):
  """
  Re-run BTD->PPR conversion for any btd_files rows that are missing weather data.
  
  This is needed for files that were converted before the weather feature was added.
  After backfilling, re-run create_master_ppr to get weather into the merged file.

  Args:
      league (str):  e.g. "NCAA"
      gender (str):  e.g. "W"
      year (int):    e.g. 2026
      team (str):    Optional - limit to one team. None = all teams.
      dry_run (bool): If True, just report what WOULD be updated without changing anything.

  Returns:
      dict: {
          'total_files':      int  - btd files found matching the filter
          'already_have_weather': int
          'needs_backfill':   int
          'backfilled':       int  - files successfully updated (0 if dry_run)
          'failed':           int
          'skipped_no_ppr':   int  - files with no ppr_data at all
          'details':          list of str
          'dry_run':          bool
      }
  """
  ...
  print(f"Starting weather backfill: {league} {gender} {year} team={team} dry_run={dry_run}")

  # Build search filters
  search_args = dict(league=league, gender=gender, year=year)
  if team:
    search_args['team'] = team

  btd_rows = list(app_tables.btd_files.search(**search_args))
  result['total_files'] = len(btd_rows)
  print(f"Found {len(btd_rows)} btd files to check")

  for flist_r in btd_rows:
    filename = flist_r['filename'] or 'unknown'

    # Skip files with no PPR data yet
    if not flist_r['ppr_data']:
      result['skipped_no_ppr'] += 1
      result['details'].append(f"SKIP (no PPR): {filename}")
      continue

    # Read the existing PPR csv for this file
    try:
      ppr_df = pd.read_csv(io.BytesIO(flist_r['ppr_data'].get_bytes()))
    except Exception as e:
      result['failed'] += 1
      result['details'].append(f"FAIL (read error): {filename} - {str(e)}")
      continue

    # Check if weather_id column exists and has data
    has_weather = (
      'weather_id' in ppr_df.columns and
      ppr_df['weather_id'].notna().any()
    )

    if has_weather:
      result['already_have_weather'] += 1
      result['details'].append(f"OK (has weather): {filename}")
      continue

    result['needs_backfill'] += 1

    # Get the required fields from btd_row
    venue_name = flist_r['venue_name'] if 'venue_name' in flist_r.keys() else None
    match_date = flist_r['date']
    match_time = flist_r['match_time'] if 'match_time' in flist_r.keys() else None

    if not venue_name or not match_date or not match_time:
      result['details'].append(
        f"SKIP (missing fields): {filename} "
    ...
    if match_time not in valid_times:
      result['details'].append(f"SKIP (bad match_time '{match_time}'): {filename}")
      result['failed'] += 1
      continue

    if dry_run:
      result['details'].append(
        f"WOULD BACKFILL: {filename} | venue={venue_name} | date={match_date} | time={match_time}"
      )
      continue

    # Fetch (or reuse cached) weather
    try:
      weather_id = get_or_create_weather(venue_name, match_date, match_time)

      if not weather_id:
        result['failed'] += 1
        result['details'].append(f"FAIL (no weather_id returned): {filename}")
        continue

      # Get the weather data to denormalize into the PPR
      weather_row = app_tables.weather_data.get_by_id(weather_id)
      if not weather_row:
        result['failed'] += 1
        result['details'].append(f"FAIL (weather row not found): {filename}")
        continue

      # Stamp all rows in the PPR dataframe with weather data
      ...
      # Save the updated PPR back to btd_files
      updated_csv = ppr_df.to_csv(index=False)
      ppr_media = anvil.BlobMedia(
        content_type="text/plain",
        content=updated_csv.encode(),
        name=filename
      )
      ...
      result['backfilled'] += 1
      result['details'].append(
        f"BACKFILLED: {filename} | weather_id={weather_id} | "
      ...
      result['failed'] += 1
      import traceback
      result['details'].append(f"FAIL (exception): {filename} - {str(e)}")
      print(f"Exception backfilling {filename}: {traceback.format_exc()}")

  # Summary
  summary = (
    f"Backfill complete: {result['backfilled']} updated, "
    f"{result['already_have_weather']} already had weather, "
    f"{result['failed']} failed, "
    f"{result['skipped_no_ppr']} skipped (no PPR)"
  )
  print(summary)
  result['details'].append(f"\n{summary}")

  if result['backfilled'] > 0 and not dry_run:
    print("Weather backfilled. Re-run create_master_ppr to update the merged file.")
    result['details'].append("NOTE: Re-run create_master_ppr to update the merged file with new weather data.")
