...
def player_attacking_as_passer_or_server( m_ppr_df, disp_player, action ):
  ...
  if m_ppr_df.shape[0] > 0:
    ...
    for i in [1,2,3,4,5,6]:
      ...
      fbhe_table.at[5,column[i-1]] = str("{:.1%}").format(oos_vector1[1])  # Out of System      
    ...
    fbhe_table.at[5,'Option'] = str("{:.1%}").format(oos_vector1[1])  # percent out of system
...
@anvil.server.callable
def player_srv_strategy(disp_league,
                      disp_gender,
                      disp_year,
                      disp_team,
                      disp_pair,
  ...
  #ppr_df = pair_filter(ppr_df, disp_pair) # lastly, filter the data to all play that include the pair of interest ...CHANGE looking at player data now
  ...
  first_zone = True
  new_ppr = ppr_df[ ppr_df['player_a1'] == ''] # the idea here is to just initialize new_ppr to an empty df
