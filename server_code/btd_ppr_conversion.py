...
def generate_ppr_files_not_background(user_league, user_gender, user_year, user_team, rebuild  ):
  ...
  for flist_r in btd_row:
    ...
    # Decide up front whether this file needs (re)processing, so we only emit
    # a CHECKPOINT for files that are actually going to run the pipeline --
    # printing "starting" for every already-current file was flooding the
    # background task's console log and pushing the real error (if any) past
    # the log's retention limit before it could ever be seen.
    needs_processing = bool(
      rebuild
      or not flist_r['ppr_file_date']
      or flist_r['btd_file_date'] > flist_r['ppr_file_date']
    )
    if needs_processing:
    ...
    try:
      ...
      # Persisted to the error_log table (unlike console output, which the
      # background task's log retention limit can truncate before this point
      # is ever reached) -- captures exactly which file broke and why.
      log_critical(
        f"generate_ppr_files_not_background crashed on '{flist_r['filename']}' "
        f"({user_league} {user_gender} {user_year} {user_team}): {e}"
      )
      raise
...
def check_last_point( ppr_df, ppr_row, btd_r, last_player, last_quality, last_action_id, last_action_type, in_trans, teama, teamb, final_pt ):
  ...
  point_code = None  # stays None if last_quality/last_action_type/team don't match any case below --
                      # matches the pre-existing behavior of leaving point_outcome unset in that situation

  if in_trans:
    # do I have a terminal quality?
    point_code = "TK" if last_team == serve_team else "TE"
    ...
    if last_quality in ["ace","error","kill"]:
      ...
      # No further touch after the pass (no set, no attack) means the rally
      # died right at the receive even though BTD didn't tag it with an
      # explicit outcome label -- score it like the serve won outright
      # (TSA), not a first-ball kill. Pass quality ratings aren't trusted
      # for this decision, only whether a further touch was recorded.
      no_further_touch = ppr_df.at[ppr_row, 'set_yn'] == 'N' and ppr_df.at[ppr_row, 'att_yn'] == 'N'
      point_code = "TSA" if (last_action_type == "serve" or no_further_touch) else "FBK"
    else:  # error, serve or first ball
      no_further_touch = ppr_df.at[ppr_row, 'set_yn'] == 'N' and ppr_df.at[ppr_row, 'att_yn'] == 'N'
      point_code = "TSE" if (last_action_type == "serve" or no_further_touch) else "FBE"

  if point_code is None:
    log_debug(
      f"check_last_point: rally_id={btd_r['rally_id']} ppr_row={ppr_row} -- no point_code matched "
      f"(last_action_type={last_action_type}, last_quality={last_quality}, last_team==serve_team={last_team == serve_team}) "
      f"-- point_outcome left unset for this point"
    )
  else:
    ppr_df = save_point( ppr_df, ppr_row, point_code, last_team, last_action_id )
