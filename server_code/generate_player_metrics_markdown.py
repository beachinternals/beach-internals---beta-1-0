...
def generate_player_metrics_markdown(league_value, team, use_direct_data=False,
                                     display_name=None, display_team=None,
                                     **json_filters):
  """
    ...
    Args:
        ...
        team (str): Team name (used for data queries - always the real team)
        use_direct_data (bool): Use direct data access (bypasses user check)
        display_name (str): Optional. If provided, used in report content instead
                            of the real player name (for de-identification).
        display_team (str): Optional. If provided, used in report content instead
                            of the real team name (for de-identification).
  ...
  try:
    ...
    # Get player name (required for data queries)
    ...
    # Resolve display names: use de-identified values if provided,
    # otherwise fall back to real values. Data queries always use real names.
    content_name = display_name if display_name else player_name
    content_team = display_team if display_team else team
    ...
    if len(ppr_df) == 0:
      ...
      # Build metadata — use content_name/content_team for display fields
    metadata = {
      'generated_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
      'player_name': content_name,        # display name (may be UUID)
      'player_shortname': player_shortname,
      'league': league,
      'gender': gender,
      'year': year,
      'team': content_team,               # display team (may be redacted)
    ...
    # Generate Markdown content — uses content_name/content_team for display
    log_info("Building Markdown output...")
    markdown_content = generate_markdown_content(
      content_name,       # display name (UUID if de-identified)
      metadata,
      metrics_result['metrics'],
      metric_dict
    )

    # Generate filename — use content_name so filename is also de-identified
    safe_name = sanitize_name_for_filename(content_name)
