from ._anvil_designer import ppr_file_maintTemplate
from anvil import *
import anvil.server
...
class ppr_file_maint(ppr_file_maintTemplate):
  def __init__(self, **properties):
    ...
    # populate the drop downs for league
    self.league_drop_down_copy.items = [
      (row["league"], row) for row in app_tables.league_list.search()
    ]
    ...
    self.league3_drop_down.selected_value = (
      user_row["def_league"] + "|" + user_row["def_gender"] + "|" + user_row["def_year"]
    )
    self.league3_drop_down.items = list(
      set(
        [
  ...
  def merge_ppr_button_click(self, **event_args):
    ...
    data_set = (
      self.drop_down_data_set_copy.selected_value
    )  # this should be: All, Private, Scouting, League

    anvil.server.call(
      "create_master_ppr", user_league, user_gender, user_year, user_team, data_set
    )
