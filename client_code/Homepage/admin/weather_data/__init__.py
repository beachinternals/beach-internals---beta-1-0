from ._anvil_designer import weather_dataTemplate
from anvil import *
import anvil.server
...
class weather_data(weather_dataTemplate):
  def __init__(self, **properties):
    ...
    self.venue_dropdown.items =  [(row["venue_name"], row) for row in app_tables.venue.search( venue_league=user_row['def_league'])]
  ...
  def calc_sw_click(self, **event_args):
    ...
    return_value = anvil.server.call(
      "calc_sw_player_clientcallable", disp_league, disp_gender, disp_year
    )
  ...
  def night_processing_button_click(self, **event_args):
    ...
    disp_gender = league_value[: str_loc - 1].strip()
    disp_year = league_value[str_loc + 1 :].strip()
    r_val = anvil.server.call(
      "night_processing_callable", disp_league, disp_gender, disp_year, True, False
    )
    pass
  ...
  def night_proc_all_click(self, **event_args):
    ...
    disp_year = league_value[str_loc + 1 :].strip()
    r_val = anvil.server.call(
      "night_processing_callable", disp_league, disp_gender, disp_year, False, True
    )
  ...
  def calc_sw_pair_click(self, **event_args):
    ...
    return_value = anvil.server.call(
      "calc_sw_pair_clientcallable", disp_league, disp_gender, disp_year
    )
