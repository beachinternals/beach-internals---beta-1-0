...
class btd_manage(btd_manageTemplate):
  ...
  def load_files(self):
    ...
    try:
      ...
      print(f"Loaded {len(files)} files, first file:", files[0] if files else "none")
      ...
      for f in files:
        ...
        print(f"File: {f.get('filename')}, players_display: {f.get('players_display')}")

      self.file_grid.items = files
      self.file_count_label.text = f"{len(files)} files found"
      # Debug: Check what the grid actually has
      print(f"Grid has {len(self.file_grid.items)} items")
